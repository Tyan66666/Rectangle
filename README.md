...
- **Per-display grid** — each display can have its own grid size (2×2, 2×3, 3×3, …) chosen from the status-bar menu. The optional **Cycle Grid** shortcut steps through grid sizes for the current display only — in Settings → **Zone Layout…** it can be restricted to exactly the checked n×n grids (2×2 up to 6×6) instead of every size.
